...
`telegram.env.example`을 참고해 로컬에 `telegram.env`를 만들면 됩니다. `telegram.env`와 `openai.env`는 `.gitignore`에 포함되어 GitHub에 올라가지 않습니다.
